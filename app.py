...
st.set_page_config(page_title="Générateur de Rapports PPTX", layout="centered")
    ...
    if st.button("Lancer la génération des rapports"):
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            ...
            for index, row in df.iterrows():
                ...
                # Extraction et conversion des données d'après les colonnes exactes du CSV
                ...
                # 2. Logique pour la recommandation stratégique et l'analyse IA
                if taux_calculé >= 100:
                ...
                # Parcours de toutes les slides et de toutes les formes textuelles (correctement indenté dans la boucle)
                            ...
                            for paragraph in shape.text_frame.paragraphs:
                                for run in paragraph.runs:
                                    ...
                                    if "{{NOM_CLIENT}}" in run.text:
                                        run.text = run.text.replace("{{NOM_CLIENT}}", nom_client)
                ...
                # Sauvegarde du fichier PowerPoint individuel de la ligne en mémoire
                pptx_buffer = io.BytesIO()
                prs.save(pptx_buffer)
                pptx_buffer.seek(0)
                
                # Ajout du PowerPoint courant dans le fichier ZIP final
                nom_fichier = f"Rapport_{nom_client}.pptx".replace(" ", "_")
                zip_file.writestr(nom_fichier, pptx_buffer.getvalue())
                
        # 4. DISPOSITIF DE TÉLÉCHARGEMENT DU RÉSULTAT (En dehors de la boucle d'écriture)
